...
max_filesystem_sectors = sectors_per_granule * max_granules + sectors_per_track
max_filesystem_bytes = max_filesystem_sectors * bytes_per_sector

directory_entry_size = 32
max_directory_entries = directory_sectors * bytes_per_sector // directory_entry_size
...
class GranuleInfo:
    ...
    @classmethod
    def fromraw(cls, n, raw_value, last_sector_bytes_used):
        ...
        if 0 <= b < 0xc0:
            return cls(
                ...
                n=n,
                next_n=b,
                sectors_used=sectors_per_granule,
                bytes_used=sectors_per_granule * bytes_per_sector,
                last=False,
                free=False,
            )
        elif 0xc0 <= b <= 0xc9:
            sectors_used = b & 0xf
            assert sectors_used <= sectors_per_granule, (n, raw_value, sectors_used)
            if sectors_used == 0:
                assert last_sector_bytes_used == 0, last_sector_bytes_used
                bytes_used = 0
            else:
                bytes_used = (sectors_used - 1) * bytes_per_sector + last_sector_bytes_used
            return cls(
                raw_value=b,
                n=n,
                next_n=None,
...
class FilesystemStats:
    ...
    sectors_per_granule:int = sectors_per_granule
...
class RSDOSFilesystem:
    ...
    def sector_count(self):
        n = self._getsize64()
        extraneous_bytes = -(n % -bytes_per_sector)
        if extraneous_bytes:
        ...
        n_sectors = n // bytes_per_sector
        if n_sectors < (directory_track + 1) * sectors_per_track:
            raise FilesystemFatalError(f"filesystem is smaller than the minimum size")

        return n_sectors

    def granule_count(self):
        s = self.sector_count() - sectors_per_track
        extraneous_sectors = -(s % -sectors_per_granule)
        ...
        return s // sectors_per_granule

    def sector_range_linear(self, n, sector_count=1):
        # This uses 0-based sector numbering
        start = n * bytes_per_sector
        end = start + bytes_per_sector * sector_count
        return (start, end)
    ...
    def sector_range(self, cyl, secnr, sector_count=1):
        ...
        if not 1 <= secnr <= sectors_per_track:
            raise IndexError("sector number out of range")
        if not 0 <= sector_count <= sectors_per_track - (secnr-1):
            raise IndexError("sector count out of range")
        return self.sector_range_linear(cyl * sectors_per_track + (secnr-1), sector_count)
    ...
    def granule_range_sectors(self, granule):
        sec_start = granule * sectors_per_granule
        sec_end = (granule + 1) * sectors_per_granule
        if sec_start >= directory_track * sectors_per_track:
            # Skip over the Directory track (track 17)
            sec_start += sectors_per_track
            sec_end += sectors_per_track
        return (sec_start, sec_end)

    def directory_range(self):
        # Track 17, sectors 3 - 11 (9 sectors)
        return self.sector_range(directory_track, 3, 9)

    def directory_entry_range(self, n):
        d_start, d_end = self.sector_range(directory_track, 3, 9)
        dent_start = d_start + n * directory_entry_size
        dent_end = dent_start + directory_entry_size
    ...
    def granule_map_range(self):
        start = self.sector_range(directory_track, 2)[0]
        end = start + min(self.granule_count(), max_granules)
        return (start, end)
    ...
    def count_granules(self):
        ...
        for i, c in enumerate(gmap):
            gi = GranuleInfo.fromraw(i, c, bytes_per_sector)
            if gi.free:
                free_count += 1
            else:
                used_count += 1
    ...
    def iter_directory_entries(self, *, include_free=False):
        for i in range(0, max_directory_entries):
            raw = self.read_directory_entry(i)
            dentry = DirectoryEntry.fromraw(i, raw)
            if dentry.null:     # HACK
                continue
    ...
    def iter_granules(self, first_granule, last_sector_bytes_used):
        ...
        while True:
            ...
            gi = GranuleInfo.fromraw(n, raw_value, last_sector_bytes_used)
            yield gi
            seen.add(n)
            if gi.last:
                break
    ...
    def compute_file_offset(self, dentry, offset):
        granule_offset, x = divmod(offset, bytes_per_sector * sectors_per_granule)
        sector_offset, byte_offset = divmod(x, bytes_per_sector)
        return (granule_offset, sector_offset, byte_offset)

    def iter_file_contents(self, dentry, length=None, offset=0):
        g0, b0 = divmod(offset, bytes_per_sector * sectors_per_granule)
    ...
    def get_file_sizes(self, dentry):
        ...
        allocated_sectors = allocated_granules * sectors_per_granule
        return logical_size, bytes_per_sector, allocated_sectors

    def get_sector_size(self):
        return bytes_per_sector

    def get_directory_size(self):
        return directory_sectors * bytes_per_sector
